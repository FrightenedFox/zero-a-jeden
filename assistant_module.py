...
sequence = ["0", "1"]															# Possible values in test sequence
worst_situaion = '101'															# In my opinion - worst possible input sequence, if repeated

class Graph:
	''' Creates a graph of the times used to process all 
	substrings using tkinter and other built-in python methods'''

	def __init__(self, width=1200, height=400, text_density=25):
		self.time_records = []
	...
	def paint_graph(self):
		''' This function creates graph after all data is collected.'''

		if self.time_records == []:												# Checking if data exist at all
			print("Za mało danych, aby narysować wykres!")
			return -1

		all_len, all_max, all_min, all_sum = [], [], [], []						# Defining common scale for all data
	...
	def graph_painter(self, times_info, number):
		''' This function draws a graph of values given in array times_info'''

		color = self.colors[number%len(self.colors)]
		x1, y1= self.legend_x, self.graph_height+self.title
		previous = 0
		for record in times_info:												# Drawing a line from previous record to current record.
...
def random_sequence(path = "input_rand.txt", lines = 10, start_length = 50, increment = 0, multiplier = 1):
	''' This function creates file filled with random sequence of digit "zero" and "one". 
...
""" Example:
path_in = '', 		- path, where new input file will be generated;
lines = 20, 		- number of strings (lines) in generated file;
start_length = 50, 	- length of the first string (line);
increment = 5,		- increment of length of the line after each line;
multiplier = 1		- multiplication of length of the line after each line.
"""
